...
@st.cache_data
def load_data():
    # Ensure all lists have exactly 120 items
    brands = [
    ...
    cuisines = ['Ramen'] * 20 + ['Sushi'] * 15 + ['Curry'] * 10 + ['Tonkatsu'] * 10 + \
               ['Yakiniku'] * 10 + ['Izakaya'] * 8 + ['Cafe'] * 6 + ['Dessert'] * 6 + \
               ['Udon'] * 4 + ['Soba'] * 4 + ['Onigiri'] * 7 + ['Donburi'] * 10 + \
               ['Okonomiyaki'] * 5 + ['Tendon'] * 2 + ['Tempura'] * 3
    ...
    # Create DataFrame ensuring all lists are the same length
    data = {
        'brand_name': brands,
        'cuisine': cuisines,
        'investment_usd': investments,
        'franchise_fee_usd': franchise_fees,
        'royalty_pct': royalties,
        'stores_japan': stores_japan,
        'stores_overseas': stores_overseas,
        'target_markets': target_markets,
        'avg_monthly_revenue_usd': monthly_revenues,
        'avg_monthly_cost_usd': monthly_costs,
        'pros': pros,
        'cons': cons
    }
